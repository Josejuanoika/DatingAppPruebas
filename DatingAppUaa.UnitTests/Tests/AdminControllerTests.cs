...
namespace DatingAppUaa.UnitTests.Pruebas
{
    public class AdminControllerTests
    {
        ...
        public async Task get_users_with_roles_ok(string statusCode, string username, string password)
        {
            // Arrange
        ...
        public async Task edit_roles_ok(string statusCode, string username, string password,string user2,string roles)
        {
            // Arrange
            var user = await LoginHelper.LoginUser(username, password);
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", user.Token);
        ...
        [Theory]
        [InlineData("OK", "admin", "Pa$$w0rd")]
        public async Task get_photos_for_moderation_ok(string statusCode, string username, string password)
        {
            // Arrange
            var user = await LoginHelper.LoginUser(username, password);
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", user.Token);
