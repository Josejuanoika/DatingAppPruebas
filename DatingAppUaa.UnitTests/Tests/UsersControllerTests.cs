...
namespace DatingAppUaa.UnitTests.Pruebas
{
    public class UsersControllerTests
    {
        ...
        public async Task get_users_no_pagination_ok(string statusCode, string username, string password)
        {
            // Arrange
        ...
        public async Task get_users_with_pagination_ok(string statusCode, string username, string password, int pageSize, int pageNumber)
        {
            // Arrange
            _client.DefaultRequestHeaders.Authorization = null;
        ...
        public async Task get_users_by_username_ok(string statusCode, string username, string password)
        {
            // Arrange
            var user = await LoginHelper.LoginUser(username, password);
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", user.Token);
        ...
        public async Task update_user_no_content(string statusCode, string username, string password,string introduction,string lookingFor,string interests, string city,string country)
        {
            // Arrange
            _client.DefaultRequestHeaders.Authorization = null;
            var user = await LoginHelper.LoginUser(username, password);
            ...
            registeredObject = get_register_object(memberUpdateDto);
        ...
        public async Task add_photo_created(string statusCode, string username, string password, string file)
        {
            // Arrange
            _client.DefaultRequestHeaders.Authorization = null;
            var user = await LoginHelper.LoginUser(username, password);
        ...
        public async Task set_main_photo_ok(string statusCode, string username, string password, string file)
        {
            // Arrange
            _client.DefaultRequestHeaders.Authorization = null;
            var user = await LoginHelper.LoginUser(username, password);
        ...
        public async Task delete_photo_ok(string statusCode, string username, string password,string file)
        {
            // Arrange
            var user = await LoginHelper.LoginUser(username, password);
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", user.Token);
        ...
        public async Task delete_photo_not_found(string statusCode, string username, string password, string id)
        {
            // Arrange
            _client.DefaultRequestHeaders.Authorization = null;
            var user = await LoginHelper.LoginUser(username, password);
        ...
        private static string get_register_object(MemberUpdateDto memberUpdateDto)
        {
        ...
        private static string get_register_object(string file)
        {
            var entityObject = new JObject()
            {
                { "File", file}
